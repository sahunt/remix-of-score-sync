CREATE OR REPLACE FUNCTION public.calculate_goal_progress(p_user_id uuid, p_level_values integer[] DEFAULT NULL::integer[], p_level_operator text DEFAULT 'is'::text, p_difficulty_values text[] DEFAULT NULL::text[], p_difficulty_operator text DEFAULT 'is'::text, p_target_type text DEFAULT 'lamp'::text, p_target_value text DEFAULT 'clear'::text)
